...
// ── Master toggles ───────────────────────────────────────────────────────────
// Each master flips canonical watweak_* UI/runtime keys through WAGRGateStore.
 // waabOn/waabOff and runtimeOn write selector/flag overrides.
static NSArray<NSDictionary *> *WAGRMasterToggles(void) {
    return @[
        ...
        @{ @"title":    @"Modo Me-Tab / Contacts Hub / About Evolve",
           ...
           @"masterKeys": @[ @"watweak_ui_metab_master_enabled" ],
...
static BOOL WAGRMasterIsOn(NSDictionary *toggle) {
    for (NSString *k in (NSArray *)toggle[@"masterKeys"]) {
        if (WAGRGateIsSet(k) && WAGRGateGet(k)) return YES;
    }
...
static void WAGRMasterApply(NSDictionary *toggle, BOOL on) {
    for (NSString *k in (NSArray *)toggle[@"masterKeys"]) {
        if (on) WAGRGateSet(k, YES);
        else    WAGRGateClear(k);
    }
