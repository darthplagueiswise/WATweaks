...
static BOOL WAGRDMIsExtraSection(id self, id tableView, NSInteger section) {
    // Kept NO intentionally. Extra rows are now injected natively via
    // WAGRDMInjectNativeRows which calls [self addSection] + WATableRow API.
    // WAStaticTableViewController manages _sections itself so section count is
    // always consistent — no manual datasource override needed.
...
// ─────────────────────────────────────────────────────────────────────────────
// WAGRDMInjectNativeRows — injects a WATweaks WATableSection into
// WADebugViewController using the exact same WAStaticTableViewController API
// that WhatsApp uses internally: -addSection, -addTableRowWithCellStyle:,
// -setHandler:, and [row cell] for label configuration.
//
// Called once per VC instance (guarded by associated object) at the end of
// -createSections so our section always appears after all native ones.
//
// API confirmed via binary analysis on SharedModules:
//   WATableSection: -init, -addRow:, -addDefaultTableRow,
//                   -addTableRowWithCellStyle:, -setHeaderText:, -setFooterText:
//   WATableRow:     -initWithCell:, -cell, -setHandler:, -setSelectionStyle:,
//                   -setDisabled:
// ─────────────────────────────────────────────────────────────────────────────
static const char *kWAGRDMNativeRowsInjected = "watweaks.dm.native.rows.injected";

static void WAGRDMInjectNativeRows(id self) {
    if (!WAGRDMIsNativeDebugVC(self)) return;

    // One injection per VC instance — createSections may be called more than once
    if ([objc_getAssociatedObject(self, kWAGRDMNativeRowsInjected) boolValue]) return;
    objc_setAssociatedObject(self, kWAGRDMNativeRowsInjected, @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);

    // ── Build the WATweaks section via WAStaticTableViewController API ────────
    SEL addSectionSel = NSSelectorFromString(@"addSection");
    if (![self respondsToSelector:addSectionSel]) {
        WAGRLogAppend(@"[NativeRows] WADebugViewController does not respond to addSection");
        return;
    }

    id section = ((id (*)(id, SEL))objc_msgSend)(self, addSectionSel);
    if (!section) { WAGRLogAppend(@"[NativeRows] addSection returned nil"); return; }

    // Header / footer
    SEL setHeader = NSSelectorFromString(@"setHeaderText:");
    SEL setFooter = NSSelectorFromString(@"setFooterText:");
    if ([section respondsToSelector:setHeader])
        ((void (*)(id, SEL, id))objc_msgSend)(section, setHeader, @"WATweaks");
    if ([section respondsToSelector:setFooter])
        ((void (*)(id, SEL, id))objc_msgSend)(section, setFooter,
            @"Acesso direto a gates, AB flags e diagnósticos sem sair do Developer Menu.");

    // Row factory — prefer UITableViewCellStyleSubtitle (= 3) for title+subtitle
    SEL addStyled    = NSSelectorFromString(@"addTableRowWithCellStyle:");
    SEL addDefault   = NSSelectorFromString(@"addDefaultTableRow");
    SEL setHandlerSel = NSSelectorFromString(@"setHandler:");
    SEL cellGetter    = NSSelectorFromString(@"cell");
    BOOL hasStyled = [section respondsToSelector:addStyled];
    BOOL hasDefault = [section respondsToSelector:addDefault];

    if (!hasStyled && !hasDefault) {
        WAGRLogAppend(@"[NativeRows] section has no row factory method");
        return;
    }

    // Row definitions: title / subtitle / row-index (index = WAGRDMOpenExtraRow arg)
    NSArray<NSArray<NSString *> *> *rowDefs = @[
        @[@"Runtime Gates / AB Flags",
          @"Categorias por feature — LiquidGlass, Aura, AI Home, Username, Online, Debug…",
          @"0"],
        @[@"WAAB Feature Keys",
          @"WAABProperties — flags do servidor em tempo real.",
          @"1"],
        @[@"Private Experimentation",
          @"Tela Swift nativa com Allocated / Fetch / Sync / Clear AB Props.",
          @"2"],
        @[@"Context / PreFlight",
          @"Cache de userContext, hooks ativos e diagnósticos.",
          @"3"],
        @[@"Logs WATweaks",
          @"Buffer de logs desta sessão.",
          @"4"],
    ];

    for (NSArray<NSString *> *def in rowDefs) {
        NSString *title    = def[0];
        NSString *subtitle = def[1];
        NSInteger rowIndex = (NSInteger)[def[2] integerValue];

        id row = nil;
        if (hasStyled) {
            // UITableViewCellStyleSubtitle = 3 — shows textLabel + detailTextLabel below
            row = ((id (*)(id, SEL, NSInteger))objc_msgSend)(section, addStyled,
                                                              (NSInteger)3 /*UITableViewCellStyleSubtitle*/);
        }
        if (!row && hasDefault) {
        ...
        // Configure the underlying UITableViewCell
        if ([row respondsToSelector:cellGetter]) {
            id cell = ((id (*)(id, SEL))objc_msgSend)(row, cellGetter);
            if ([cell isKindOfClass:[UITableViewCell class]]) {
                UITableViewCell *tc = (UITableViewCell *)cell;
                tc.textLabel.text       = title;
                tc.detailTextLabel.text = subtitle;
                tc.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
                tc.detailTextLabel.numberOfLines = 2;
            }
        }

        // Tap handler block — called by WATableRow when the user taps the row
        if ([row respondsToSelector:setHandlerSel]) {
            __weak id weakSelf = self;
            void (^handler)(void) = ^{
                id strongSelf = weakSelf;
                if (strongSelf) {
                    WAGRGateHooksEnsureInstalled();
                    WAGRDMOpenExtraRow(strongSelf, rowIndex);
                }
            };
            ((void (*)(id, SEL, id))objc_msgSend)(row, setHandlerSel, handler);
        }
    }

    WAGRLogAppendF(@"[NativeRows] injected %lu rows into WADebugViewController via WATableSection/WATableRow",
                   (unsigned long)rowDefs.count);
}
...
static void hookDMCreateSectionsVoid(id self, SEL _cmd) {
    ...
    if (orig) orig(self, _cmd);
    // ── Inject WATweaks section using native WATableSection/WATableRow API ───
    // Must be AFTER %orig so native sections are built first. Uses
    // [self addSection] → WAStaticTableViewController internal API.
    WAGRDMInjectNativeRows(self);
