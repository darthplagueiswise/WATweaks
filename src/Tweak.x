...
//     is the activation path for the WATweaks menu when the user presses-and-holds
//     the Help / Developer cells in Settings. This must stay here because the wagr_validate_sources.py
...
//     attach the long-press recognizer. WATweaks no longer injects rows or buttons
//     into WhatsApp Settings / Developer menus.
//
...
// Native WhatsApp Settings rows shim. This remains as a no-op compatibility
// surface so old callers link, but it must not inject a WATweaks row/button.
...
// We accept only native user-facing cells as triggers: "Ajuda" / "Help" or
// "Developer". There is no injected WATweaks row in WhatsApp native menus.
