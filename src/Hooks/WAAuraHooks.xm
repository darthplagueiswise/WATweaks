// WAAuraHooks.xm — Aura / WA Plus gating (FULLY MIGRATED to WAGate* + WAPref)
...
static BOOL WAGRAuraActive(void) {
    return WAPref(kWAGateAuraSimulation) || WAGateIsOn(@"aura_enabled") || WAGateIsOn(@"aura_subscription_simulation_enabled");
}
...
static BOOL hook_aura_positive(id self, SEL _cmd) {
    ...
    if (WAGateIsSet(sel)) return WAGateGet(sel);
...
static NSArray<NSString *> *WAGRAuraClasses(void) {
    return @[ @"WAAuraGating", @"_TtC12WAAuraGating20GatedBenefitProvider", @"_TtC12WAAuraGating25GatedSubscriptionProvider", @"_TtC12WAAuraGating14AuraBenefitReliabilityLogger" ];
}
...
extern "C" NSString *WAGRAuraDiagnostic(void) {
    return [NSString stringWithFormat:@"simulation=%@\naura_enabled_override=%@\nWAAuraGating=%@\nhooks=%lu", WAGRAuraActive()?@"ON":@"OFF", WAGateIsSet(@"aura_enabled")?@"SET":@"NOT SET", NSClassFromString(@"WAAuraGating")?@"found":@"missing", (unsigned long)gAuraOrig.count];
}
